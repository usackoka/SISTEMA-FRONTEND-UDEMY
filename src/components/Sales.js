...
class Sales extends Component{
    ...
    constructor(props){
        ...
        this.headers = [
            ['Usuario','Cliente','Tipo Comprobante','Serie Comprobante','Numero Comprobante','Total','Estado','Impuesto'],
    ...
    handleSwitchActivate(id){
        const pathQuery = '/venta/query?_id=';
        this.Requests.query(pathQuery+id).then(response=>{
            //Actualizar tabla activa
            const pathActDeactivate = ('/venta/')+((response.estado && response.estado==1)?"deactivate":"activate");
